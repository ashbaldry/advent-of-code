...
good_valves_comb <- lapply(seq(good_valves), \(x) {
  ...
  unlist()
first_opens <- matrix(Inf, length(good_valves_comb) + 1, length(valves), dimnames = list(c("", good_valves_comb), valves))
...
find_potential_power <- function(x = valves[1], i = 1, n = 30, pressure = 0, curr_open = character(0)) {
  ...
  if (identical(curr_open, good_valves)) {
    return(find_potential_power(valves[1], i = i + 1, n = n, curr_open = curr_open, pressure = pressure))
  }

  mem_name <- paste(paste0(curr_open, collapse = ""), i, x, sep = "_")
  if (mem_name %in% names(max_mem_values)) {
    if (max_mem_values[[mem_name]] == x) curr_open <- sort(union(x, curr_open))
    return(find_potential_power(max_mem_values[[mem_name]], i = i + 1, n = n, curr_open = curr_open, pressure = pressure))
  }
  ...
  max_mem_values <<- c(
    max_mem_values,
    setNames(
      move_vals[which.max(end_power)],
      paste(paste0(curr_open, collapse = ""), i, x, sep = "_")
    )
  )
  max(end_power)
}
...
max_mem_values[grep("_2_", names(max_mem_values))]
